...
What other organizations were identified by email address? Was the contribution significant enough to impact the results of our assumption?

Overall only a small proportion of commits came from external organizations. Some of these are from universities which could indicate internships. Others could be from forks of external repositories. Further analysis is needed to be sure, but the proportion is below 5% of all commit activity.
...
project_engagement <- project_engagement %>% 
  ...
  mutate(months_pct = round(activity_months/activity_months_max, 1),
         engagement_score = months_pct + commit_weight/50) %>% # adjust the weight so it fits better with percent
  # add company type
...
This section focuses on Microsoft and IBM to test the engagement score. 
...
Where does IBM's engagment overlap and how does it compare with other companies? According the plot below, 
...
ggplot(engagement_compared_to_ibm %>% filter(commit_access == "open"),
  ...
  geom_jitter (aes(group=project, color=project)) +
  guides(color=guide_legend(ncol=1)) +
  theme_classic() +
  scale_color_manual(values=ggplot_scale) +
  facet_wrap(~ company_name) +
  labs(x="Engagement Score", y="IBM Engagement Score")
